...
type LabelSize = NonNullable<LabelOwnProps['size']>;
...
const normalizeSize = (size: LabelOwnProps['size']): LabelSize => {
  if (size === 'small') return 'small';
  if (size === 'large') return 'large';
  return 'medium';
};

const sizeClasses = (size: LabelOwnProps['size']): string =>
  TEXT_SIZE_CLASS_MAP[normalizeSize(size)];

export type LabelClassNameInput = Pick<LabelOwnProps, 'size' | 'disabled'> & {
  className?: string;
};

export const getLabelClassName = ({ size, disabled, className }: LabelClassNameInput): string =>
  joinClassNames(
    LABEL_BASE_CLASS,
    sizeClasses(size),
